	...
	for _, group := range g.Groups {
		for _, version := range group.Versions {
			...
			if g.CreateRegistry {
				...
				imports = append(imports, strings.ToLower(fmt.Sprintf("%s \"%s\"", groupAlias, filepath.ToSlash(path.Vendorless(packagePath)))))
				break
			} else {
				imports = append(imports, strings.ToLower(fmt.Sprintf("%s%s \"%s\"", groupAlias, version.Version.NonEmpty(), filepath.ToSlash(path.Vendorless(packagePath)))))
			}
		}
	}
	return
